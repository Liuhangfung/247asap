...
CHANNEL_USERNAME = "@automatedpost"
...
try:
    ...
    if order:
        ...
        print(f"Billing Name: {order['billing_name']}")
        print(f"Billing Address: {order['billing_address']}")
        print(f"Billing Email: {order['billing_email']}")
        print(f"Billing Phone: {order['billing_phone']}")
        print(f"Shipping Name: {order['shipping_name']}")
        print(f"Shipping Address: {order['shipping_address']}")
        print(f"Shipping Country: {order['shipping_country']}")
        print(f"Shipping Postcode: {order['shipping_postcode']}")
        ...
        if metadata:
            try:
                ...
                print("\nAdditional Metadata (only non-empty values):")
                for key, value in parsed_metadata.items():
                    # Replace metadata key with question if available
                    question = questions_map.get(key, key)  # Default to key if no question exists
    ...
    if postmeta:
        ...
        print(f"\nID Card URL: {full_url}")
    else:
        print("\nNo meta value found in the wp_postmeta table.")
